...
class Face(object):
    ...
    def __init__(self):
        ...
        self._leftEyeRect = None
        self._rightEyeRect = None
        self._noseRect = None
        self._mouthRect = None
        
        self.faceImage = None
        
        self.detectOrganCount=0
        
    @property
    def leftEyeRect(self):
        return self._leftEyeRect
    @leftEyeRect.setter
    def leftEyeRect(self,rect):
        if(rect is not None):
            self._leftEyeRect=rect
            self.detectOrganCount+=1
    @property
    def rightEyeRect(self):
        return self._rightEyeRect
    @rightEyeRect.setter
    def rightEyeRect(self,rect):
        if(rect is not None):
            self._rightEyeRect=rect
            self.detectOrganCount+=1
            
    @property
    def noseRect(self):
        return self._noseRect
    @noseRect.setter
    def noseRect(self,rect):
        if(rect is not None):
            self._noseRect=rect
            self.detectOrganCount+=1
    @property
    def mouthRect(self):
        return self._mouthRect
    @mouthRect.setter
    def mouthRect(self,rect):
        if(rect is not None):
            self._mouthRect=rect
            self.detectOrganCount+=1
        
        
        
    def logging_faceZone(self,frame):
        if self.faceRect is not None:
            x,y,w,h=self.faceRect
            self.faceImage=frame[y:y+h,x:x+w]
        else:
            logging.debug('No faceRect in the Frame')
...
if __name__=='__main__':
    ...
    faceDet.update(img)
    winname='DebugFaceTracker'
    cv2.namedWindow(winname)
    faceDet.drawDebugRects(img)
