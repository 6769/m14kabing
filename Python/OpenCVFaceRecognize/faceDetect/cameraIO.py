...
from previewWindowManager import PreviewWindowManager
import logging
...
class IOSteam(object):
    ...
    def __init__(self,channel=0,queue_len=30,subthreadName='cameraIO'):
        ...
        self._jump_init_camera_frame=30
    ...
    def dumpFrame(self,direct='./',filename=str(int(  time.time()  ))  ,ext='.png'):
        randomstr=str(random.random())[2:7]
        sp='_'
        filename=direct+filename+sp+str(self._dumped_Frame_count)+sp+randomstr+ext
        if self._keep_fetch and self._threading_obj.isAlive():
            safeFrame=self.framequeue[-1]
            
            if cv2.imwrite(filename,safeFrame):
